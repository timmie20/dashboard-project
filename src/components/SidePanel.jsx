...
const SidePanel = () => {
  ...
  return (
    <>
      <aside
        className={`hidden lg:block ${isExpanded ? "w-full" : "w-fit"} relative h-full overflow-hidden transition-all duration-700 ease-in-out`}
      >
        <nav className="flex h-full flex-col gap-10 bg-white px-8 py-4 shadow-sm">
          <div
            className="absolute right-2 top-7 z-20 cursor-pointer "
            onClick={() => setIsExpanded(!isExpanded)}
          >
            {isExpanded ? (
              <CiCircleChevLeft
                size={26}
                className="rounded-full bg-[#EEF0FA] text-black"
              />
            ) : (
              <CiCircleChevRight
                size={26}
          ...
          <div className="flex flex-1 flex-col">
            ...
            <Link className="nav_links relative">
              ...
              <div
                className={`absolute top-3 size-5 rounded-full bg-[#FFCC91] text-center text-sm ${!isExpanded ? "left-[40px]" : "right-[80px]"}`}
              >
                3
              </div>
            </Link>
          ...
          <div className="text-gray-clr-medium">
            <div className=" flex w-full items-center gap-3 rounded-2xl bg-gray-clr-light px-4 py-[11px]">
              ...
              {isExpanded && <p className="text-sm">Contact Support</p>}
            </div>
            ...
            <div className="mt-4 flex w-full flex-col gap-4 rounded-2xl bg-orange-clr-full bg-opacity-20 px-4 py-[11px]">
              <span className="flex items-center gap-3">
                ...
                {isExpanded && <p className="text-sm">Free Gift Awaits You!</p>}
              </span>
              {isExpanded && (
